...
use crate::protocol::{CURSOR_COLORS, CursorShape, CursorState, MultiCursorMode};
...
impl CursorTracker {
    ...
    pub fn should_inject_input(&self, cursor_id: &str) -> bool {
        match self.mode {
            MultiCursorMode::Collaborative => self.active_cursor.as_deref() == Some(cursor_id),
            MultiCursorMode::TechControl => {
                // Remote cursors (non-local) have control
                self.cursors.get(cursor_id).is_some_and(|c| !c.is_local)
            }
            MultiCursorMode::UserControl => {
                // Only local cursor has control
                self.cursors.get(cursor_id).is_some_and(|c| c.is_local)
            }
            MultiCursorMode::RequestControl => self.active_cursor.as_deref() == Some(cursor_id),
        }
    }
...
pub fn generate_arrow_sprite(hex_color: &str) -> CursorSprite {
    ...
    let arrow_shape: &[(u32, u32)] = &[
        ...
        (0, 3),
        (5, 7), // row 13 (two segments handled below)
        (0, 3),
        (6, 8), // row 14
        (0, 2),
        (7, 9), // row 15
