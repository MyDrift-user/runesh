...
pub fn parse_netflow_v5(data: &[u8]) -> Result<(NetflowV5Header, Vec<FlowRecord>), String> {
    ...
    let base_time =
        DateTime::from_timestamp(unix_secs as i64, unix_nsecs).unwrap_or_else(|| Utc::now());
    ...
    for i in 0..count as usize {
        ...
        let first_uptime = u32::from_be_bytes([r[24], r[25], r[26], r[27]]);
        let last_uptime = u32::from_be_bytes([r[28], r[29], r[30], r[31]]);
...
pub async fn listen_netflow_v5<F>(bind_addr: &str, mut on_flows: F) -> Result<(), std::io::Error>
where
    F: FnMut(NetflowV5Header, Vec<FlowRecord>),
{
    let socket = tokio::net::UdpSocket::bind(bind_addr).await?;
    tracing::info!(%bind_addr, "NetFlow v5 collector listening");
